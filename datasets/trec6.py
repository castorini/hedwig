...
class TREC6(TabularDataset):
    NAME = 'TREC6'
    NUM_CLASSES = 6
    IS_MULTILABEL = False

    TEXT_FIELD = Field(batch_first=True, tokenize=clean_string, include_lengths=True)
    LABEL_FIELD = Field(sequential=False, use_vocab=False, batch_first=True, preprocessing=lambda s: process_labels(s, TREC6.NUM_CLASSES))
    ...
    @classmethod
    def splits(cls, path, train=os.path.join('.local_data', 'TREC6', 'train.csv'),
               test=os.path.join('.local_data', 'TREC6', 'test.csv'), **kwargs):
        return super(TREC6, cls).splits(
...
def char_quantize_trec6():
    return char_quantize_class(TREC6CharQuantized)


class TREC6CharQuantized(TREC6):
    ALPHABET = ALPHABET_DICT
    TEXT_FIELD = Field(sequential=False, use_vocab=False, batch_first=True, preprocessing=char_quantize_trec6)
